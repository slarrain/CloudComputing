...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <!-- Minimum required: S3 key, acl, policy, signature, AWS Access Key -->
      ...
      <input type="hidden" name="key" value="{{aws_username}}/${filename}">
      <input type="hidden" name="acl" value="public-read">
      <input type="hidden" name="success_action_redirect" value="http://54.174.243.246:8888/annotator">
      <input type="hidden" name="policy" value={{policy_encod}}>
      <input type="hidden" name="signature" value={{signat}}>

      VCF input file: <input id="upload_file" type="file" name="file" />
      <hr>
      <input type="submit" value="Run Annotator" />
    </form>
  </body>
</html>
